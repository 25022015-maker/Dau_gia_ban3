...
public class ThisBiddingController implements Initializable {
    ...
    public void handlePlaceBid(ActionEvent event) {
        ...
        if (amountText.isEmpty()) {
            bidHistoryList.getItems().add(0, "⚠️ Vui lòng nhập giá muốn đặt!");
            return;
        }
        ...
        try {
            ...
            bidHistoryList.getItems().add(0, "⚠️ Giá không hợp lệ: " + amountText);
            return;
        }

        NetworkClient client = SessionManager.getNetworkClient();
        if (client == null || !client.isConnected()) {
            bidHistoryList.getItems().add(0, "⚠️ Chưa kết nối với Server! Không thể đặt giá.");
            return;
        }

        String bidderId = SessionManager.getCurrentUser();
        if (bidderId == null || bidderId.isEmpty()) {
            bidHistoryList.getItems().add(0, "⚠️ Lỗi: Bạn chưa đăng nhập!");
            return;
        }

        if (currentAuctionId == null) {
            bidHistoryList.getItems().add(0, "⚠️ Lỗi: Không xác định được ID phiên đấu giá!");
            return;
        }
        ...
        // Disable nút tạm thời 1 giây tránh spam click
        btnPlaceBid.setDisable(true);
        new Timeline(new KeyFrame(javafx.util.Duration.seconds(1),
